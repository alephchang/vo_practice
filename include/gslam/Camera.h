...
public:
    ...
    float   fx_, fy_, cx_, cy_, depth_scale_, base_line_;

    Camera();
    Camera ( float fx, float fy, float cx, float cy, float depth_scale=0 ,float base_line=0) :
        fx_ ( fx ), fy_ ( fy ), cx_ ( cx ), cy_ ( cy ), depth_scale_ ( depth_scale ), base_line_(base_line)
    {}
